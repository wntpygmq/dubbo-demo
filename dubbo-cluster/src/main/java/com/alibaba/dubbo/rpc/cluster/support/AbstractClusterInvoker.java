...
public abstract class AbstractClusterInvoker<T> implements Invoker<T> {
     ...
     *
     *      注意：若 reselect 选出来的 Invoker 为空，此时定位 invoker 在 invokers 列表中的位置 index，
    ...
    /**
     * 集群工作过程可分为两个阶段，
     * 第一个阶段是在服务消费者初始化期间，这个在服务引用那篇文章中分析过，就不赘述。
     * 第二个阶段是在服务消费者进行远程调用时，此时 AbstractClusterInvoker 的 invoke 方法会被调用
     *
     * @return
     * @throws RpcException
     */
    @Override//触发时机是消费者初始化和调用方法
    public Result invoke(final Invocation invocation) throws RpcException {
        ...
        if (invokers != null && !invokers.isEmpty()) {
            loadbalance = ExtensionLoader.getExtensionLoader(LoadBalance.class).getExtension(invokers.get(0).getUrl()
                    .getMethodParameter(RpcUtils.getMethodName(invocation), Constants.LOADBALANCE_KEY, Constants.DEFAULT_LOADBALANCE));//random
        }
        RpcUtils.attachInvocationIdIfAsync(getUrl(), invocation);
        // 调用子类（默认是FailoverClusterInvoker）的doInvoke 进行后续操作.
