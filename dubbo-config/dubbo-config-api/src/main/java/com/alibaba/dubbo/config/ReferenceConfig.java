...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    private void init() {
        ...
        // get consumer's global configuration// 检测 consumer 变量是否为空，为空则创建
        checkDefault();
        appendProperties(this);
        if (getGeneric() == null && getConsumer() != null) {
            // 设置 generic，泛接口调用方式主要用于客户端没有API接口及模型类元的情况，参数及返回值中的所有POJO均用Map表示
     ...
     * 若不是，则读取直连配置项，或注册中心 url，并将读取到的 url 存储到 urls 中。然后根据 urls 元素数量
     * 进行后续操作。若 urls 元素数量为1，则直接通过 Protocol 自适应拓展类构建 Invoker 实例接口。若 urls
     * 元素数量大于1，即存在多个注册中心或服务直连 url，此时先根据 url 构建 Invoker。然后再通过 Cluster
    ...
    private T createProxy(Map<String, String> map) {
        ...
        // 远程引用
            ...
            if (urls.size() == 1) {
                ...
                zookeeper://47.98.213.18:2181/com.alibaba.dubbo.registry.RegistryService?anyhost=true&application=demo-consumer&bean.name=com.alibaba.dubbo.demo.DemoService&check=false&delay=-1&dubbo=2.0.2&generic=false&interface=com.alibaba.dubbo.demo.DemoService&methods=sayHello&pid=6657&register.ip=192.168.6.48&remote.timestamp=1583218891102&side=consumer&timestamp=1583220068829

                DubboProtocol
                interface com.alibaba.dubbo.demo.DemoService ->
                dubbo://192.168.6.48:20880/com.alibaba.dubbo.demo.DemoService?anyhost=true&application=demo-consumer&bean.name=com.alibaba.dubbo.demo.DemoService&check=false&delay=-1&dubbo=2.0.2&generic=false&interface=com.alibaba.dubbo.demo.DemoService&methods=sayHello&pid=6657&register.ip=192.168.6.48&remote.timestamp=1583218891102&side=consumer&timestamp=1583220068829
