...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
     ...
     * 5 根据register和subscribe参数，设置服务是否注册和消费是否订阅
     * @param provider
     * @return
     */
    protected List<URL> loadRegistries(boolean provider) {
            ...
            for (RegistryConfig config : registries) {
                ...
                if (address.length() > 0 && !RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    ...
                    for (URL url : urls) {
                        ...
                        if ((provider && url.getParameter(Constants.REGISTER_KEY, true))//是否注册register="false"
                                || (!provider && url.getParameter(Constants.SUBSCRIBE_KEY, true))) {//是否订阅subscribe="false"
        ...
        //url对象的列表registry://47.98.213.18:2181/com.alibaba.dubbo.registry.RegistryService?application=demo-provider&dubbo=2.0.2&pid=1177&registry=zookeeper&timestamp=1583070421154
        return registryList;
    }
