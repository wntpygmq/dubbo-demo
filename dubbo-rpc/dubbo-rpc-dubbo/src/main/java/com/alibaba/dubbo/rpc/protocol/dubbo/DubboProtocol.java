...
public class DubboProtocol extends AbstractProtocol {
    ...
    private ExchangeHandler requestHandler = new ExchangeHandlerAdapter() {
        ...
        //处理HeaderExchangeHandler#handleRequest
        @Override
    ...
    Invoker<?> getInvoker(Channel channel, Invocation inv) throws RemotingException {
        ...
        // 服务导出过程中（this.export(Invoker<T> invoker)）会将 <serviceKey, DubboExporter> 映射关系存储到 exporterMap 集合中
    ...
    private ExchangeServer createServer(URL url) {
        ...
        try {
            // 创建 ExchangeServer，最后会使用neety打开服务器，bind端口
    ...
    public <T> Invoker<T> refer(Class<T> serviceType, URL url) throws RpcException {
        optimizeSerialization(url);
        // create rpc invoker.
        //getClients这个方法用于获取客户端实例，实例类型为 ExchangeClient。ExchangeClient 实际上并不具备通信能力，它需要基于更底层的客户端实例进行通信。比如 NettyClient
    ...
    private ExchangeClient getSharedClient(URL url) {
        ...
        synchronized (locks.get(key)) {
            ...
            // 将 ExchangeClient 实例传给 ReferenceCountExchangeClient，创建一个带有引用计数功能的 ExchangeClient 实例，这里使用了装饰模式
     ...
     * 最后根据 lazy 配置决定创建什么类型的客户端。这里的 LazyConnectExchangeClient 代码并不是很复杂，该类会在 request
     * 方法被调用时通过 Exchangers 的 connect 方法创建 ExchangeClient 客户端
     */
